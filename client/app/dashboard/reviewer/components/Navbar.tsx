...
const NavItemMobile: React.FC<NavItemPropsMobile> = ({ label, active, address}) => (
  <Link href={`/dashboard/reviewer${address}`} className={`group relative flex items-center text-black justify-center w-full h-12 rounded-lg cursor-pointer transition gap-[12px] p-[12px]  ${active && " text-primary-500 "}`}>
   <div>
   {label}
   </div>
  </Link>
