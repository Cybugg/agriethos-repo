...
export default function CropReviewPage() {
  ...
  useEffect(() => {
    const fetchCropData = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get(`https://agriethos-9wy5.onrender.com/api/crops/${id}`);
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await axios.put(`https://agriethos-9wy5.onrender.com/api/crops/${id}`, {
