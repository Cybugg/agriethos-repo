...
import { useNavContext } from "../NavContext"; // Add this import
...
export default function StatisticsPage() {
  ...
  const { setCurrentPage, setMobileDisplay } = useNavContext(); // Add this line

  // Set the current page when component mounts
  useEffect(() => {
    setCurrentPage("statistics"); // This will update the active tab
  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        ...
        const reviewedResponse = await axios.get(
          "http://localhost:5000/api/crops/reviewed"
        );
        ...
        if (reviewedResponse.data.success) {
          ...
          const verified = reviewedCrops.filter(
            (crop: { verificationStatus: string }) =>
              crop.verificationStatus === "verified"
          ).length;
          const rejected = reviewedCrops.filter(
            (crop: { verificationStatus: string }) =>
              crop.verificationStatus === "rejected"
          ).length;
          const toUpgrade = reviewedCrops.filter(
            (crop: { verificationStatus: string }) =>
              crop.verificationStatus === "toUpgrade"
          ).length;
          ...
          const activityData: ((prevState: never[]) => never[]) | {
            name: string;
            value: any;
          }[] = [];
          dayNames.forEach((day) => {
          ...
          reviewedCrops.forEach(
            (crop: {
              farmPropertyId: { farmName: any; _id: any };
    ...
    <div className="flex h-screen bg-white">
      ...
      <div className="flex-1 overflow-auto mt-[55px]">
        <header className="flex justify-between items-center p-4 md:p-6 border-b border-[#cfcfcf]">
          <div>
            <h1 className="text-xl md:text-2xl font-semibold text-[#000000]">
              Statistics
            </h1>
            <p className="text-lg text-[#898989]">
              View your review performance and metrics
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              className="md:hidden p-2 rounded-full hover:bg-[#f6fded]"
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mb-6 ">
                <div className="bg-white border border-[#cfcfcf] p-4 md:p-6">
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-[#149414]">
                    {summaryStats.cropsApproved}
                  </h2>
                </div>
                <div className="bg-white border border-[#cfcfcf] p-4 md:p-6">
                  <p className="text-[#898989] text-sm mb-2">Crops Rejected</p>
                  <h2 className="text-2xl md:text-3xl font-bold text-[#e30e0e]">
                    {summaryStats.cropsRejected}
                  </h2>
                </div>
                <div className="bg-white border border-[#cfcfcf] p-4 md:p-6 sm:col-span-2 md:col-span-1">
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-[#003024]">
                    {summaryStats.totalCropsReviewed}
                  </h2>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-white rounded-lg border border-[#cfcfcf] p-4 md:p-6">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="text-base md:text-lg font-medium text-[#000000]">
                      Review Activity
                    </h3>
                    <button
                ...
                {/* Pie Chart */}
                <div className="bg-white rounded-lg border border-[#cfcfcf] p-4 md:p-6">
                  <h3 className="text-base md:text-lg font-medium text-[#000000] mb-4">
                  ...
                  <div className="h-[180px] md:h-[250px] flex flex-col items-center justify-center">
                    ...
                    <div className="flex gap-6 mt-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs md:text-sm text-[#898989]">
                          Verified
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                  ...
                  <table className="min-w-full">
                    <thead>
                      <tr className="text-left bg-[#f9f9f9]">
                        <th className="px-6 py-3 text-sm font-medium text-[#898989]">
                          Farm Name
                        </th>
                        <th className="px-6 py-3 text-sm font-medium text-[#898989]">
                          Crops Reviewed
                    ...
                    <tbody>
                      {topFarmsData.map((farm) => (
                        <tr
                          key={farm.id}
                          className="hover:bg-[#f6fded] border-t border-[#f0f0f0]"
                        >
